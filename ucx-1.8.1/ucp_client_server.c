...
#include <time.h>
int message_len = 100;
#define TEST_STRING_LEN        1000000
                            ...
                            int current_iter)
{
    ...
    if (!is_server) {
        ...
        request = ucp_stream_send_nb(ep, test_message, 1,
                                     ucp_dt_make_contig(message_len),
                                     send_cb, 0);
    } else {
                         ...
                         int current_iter)
{
    ...
    if (!is_server) {
        ...
        request = ucp_tag_send_nb(ep, test_message, 1,
                                  ucp_dt_make_contig(message_len), TAG,
                                  send_cb);
    } else {
                                 ...
                                 send_recv_type_t send_recv_type, int is_server)
{
    int i, ret = 0;
    message_len=1;
    test_message = malloc(1000000);
    for (message_len = 1024; message_len<=1000000; message_len=message_len<<1){

            
            memset(test_message,'a',message_len);
            ...
            printf("%d\t%f\tGbps\n",message_len,(double)(message_len*num_iterations) *8 /   ((double)(end.tv_nsec - begin.tv_nsec)));
